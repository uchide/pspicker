...
class PSpickerDataset(model.Dataset):
    ...
    def load_sac(self, sac_info,shape=[10,12000,3],add_sub=True):
        ...
        for window_id,main_event in enumerate(sac_info["windows"]):
            ...
            self.add_window("pspicker", window_id=window_id, 
                            main_stations=main_stations,  sub_stations=sub_stations,
                            main_name=main_event["name"], sub_name=sub_event["name"], 
                            main_match=main_match,        sub_match=sub_match, 
